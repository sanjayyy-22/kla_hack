...
def solve_tsp_greedy(distance_matrix):
    ...
    visited = [False] * n_neighbourhoods
    path = [0]  # Start from the first neighborhood
    total_distance = 0

    for _ in range(n_neighbourhoods - 1):
        current = path[-1]
        next_neighbour = min((i for i in range(n_neighbourhoods) if not visited[i]), key=lambda x: distance_matrix[current][x])
...
def generate_output_path(path, start_point, distance_matrix):
    output = {
        "v0": {
            "path": [start_point] + [f'n{index}' for index in path] + [start_point],
        }
    }
    return output
...
def main():
    ...
    shortest_path, total_distance = solve_tsp_greedy(distance_matrix)

    # Generate output format
    output_data = generate_output_path(shortest_path, "r0", distance_matrix)
    output_json_file = "output0.json"
